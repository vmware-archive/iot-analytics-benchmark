#! /usr/bin/python
"""
 iotstream_lr.py: Spark streaming program to analyze data generated by sim_sensors_lr.py with Logistic Regression
   Outputs prediction of model based on input data

 In one window:
    $ python sim_sensors_lr.py | nc -lk <Port of stream source>
...
if len(sys.argv) != 8:
    print("Usage: iotstream_lr.py n_sensors reporting_interval IP_address_of_stream_source Port_of_stream_source HDFS_or_S3 HDFS_path_or_S3_bucket modelname", file=sys.stderr)
    exit(-1)
...
print('%s.%03dZ: Analyzing stream of input from host %s on port %d using LR model %s, with %.1f second intervals' % (strftime("%Y-%m-%dT%H:%M:%S", gmtime()), (time()*1000)%1000, IP_address, port, modelname, reporting_interval))
...
# This code combines the lines in each batch into a feature vector containing the latest sensor values
  ...
  # 2017-12-14T22:22:43.895Z,19,Sensor 19,0.94764
  ...
  #  create a list of tuples with each tuple representing (sensor number, sensor value)
    input = rdd.map(lambda line: line.split(',')).filter(lambda list: len(list) == 4).map(lambda list: (int(list[1]), float(list[3]))).collect()
...
sc = SparkContext(appName="iotstream_lr")
...
sensor_stream = ssc.socketTextStream(IP_address, port)
