...
if len(sys.argv) != 4:
    print("Usage: python sim_sensors_lr.py n_sensors average_sensor_events_per_second total_events", file=sys.stderr)
    exit(-1)
...
print("%sZ: Generating %d sensor events per second representing %d sensors for a total of %d events" % (strftime("%Y-%m-%dT%H:%M:%S", gmtime()), events_per_second, n_sensors, total_events), file=sys.stderr)
...
for i in range(total_events):
  ...
  if ((i+1) % events_per_second == 0): 
    print("%sZ: %d events sent" % (strftime("%Y-%m-%dT%H:%M:%S", gmtime()), i+1), file=sys.stderr)

# Send negative sensor number to indicate end of stream
simulateSensorEvent(-1,0)

print("%sZ: Sensor stream ended - keeping socket open for 1000 seconds" % (strftime("%Y-%m-%dT%H:%M:%S", gmtime())), file=sys.stderr)
