#! /usr/bin/python
"""
iotstream_lr_mqtt.py: Spark streaming program to analyze data generated by sim_sensors_lr_mqtt.py with Logistic Regression using MQTT input
  Outputs prediction of model based on input data

In one window:
  $ python sim_sensors_lr_mqtt.py n_sensors average_sensor_events_per_second total_events mqtt_server mqtt_port mqtt_topic

In another window:
  $ spark-submit iotstream_lr_mqtt.py n_sensors reporting_interval mqtt_server mqtt_port mqtt_topic HDFS_or_S3 HDFS_path_or_S3_bucket modelname
...
mqtt_URL = "tcp://{}:{}".format(sys.argv[3], sys.argv[4])
mqtt_topic = sys.argv[5]

if sys.argv[6].capitalize() == 'S3':
  modelname = "s3a://{}/{}".format(sys.argv[7], sys.argv[8])
else:
